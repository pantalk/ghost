#!/usr/bin/env bash
#
# Report how much of the image is the graphical desktop substrate.
#
# The graphical stack is measured as the dependency closure of the desktop
# top-level packages: whatever apt would remove if they were purged. This
# accounts for shared libraries pulled in transitively, which a naive sum of
# package sizes misses.
#
# Usage: tools/size-report.sh [IMAGE]
set -euo pipefail

IMAGE="${1:-${IMAGE:-pantalk/ghost:local}}"
...
echo "Image: $IMAGE"
awk -v b="$total_bytes" \
    'BEGIN {printf "Total size: %.2f GB (%.0f MiB)\n", b/1e9, b/1048576}'
echo
echo "Largest layers"
echo "--------------"
...
awk 'NR==FNR {want[$1]=1; next}
     want[$1] {
       ...
       else if (p ~ /^(xserver|xorg|x11|xauth|xinit|xkb|libx|libxcb|xterm|xdotool|wmctrl|scrot|libxcvt)/) c="X11 server and utilities"
       else if (p ~ /^(openbox|obconf|tint2|picom|feh|libimlib)/) c="Window manager, panel, compositor"
       else if (p ~ /^kitty/)                          c="kitty terminal"
       else if (p ~ /^ranger/)                         c="ranger file manager"
